...
（注：这里询问多了一个属性 $q.current$，表示这个询问已经“拥有”的贡献。）
...
第二种算法实际上就是维护一个全局的贡献，先加上指标不超过 $mid$ 的修改的贡献，然后递归解决右半部分，然后将这些修改回退，再去解决左半部分。
...
1. 整体地计算多个修改对多个询问的贡献。
2. 算法 1 的 21 行处 / 算法 2 的 $current$ 全局变量，将已经应用了的修改造成的贡献存下来，之后不再重复计算。
